...
#define BOTNICK "megahal"
#define BOTNICK2 "megahal:"
...
static void regex_filter(char *pattern, char *text)
{
	pcre *regexp;
	const char *error;
	int erroffset;
	...
	if (!regexp) {
		putlog(LOG_MISC, "*", "<ERROR> pcre_compile(): Error in pattern (%s) at %d: '%s'", pattern, erroffset, error);
		return;
	}

	int ovector[60];
	int result = pcre_exec(regexp, NULL, text, strlen(text), 0, 0, ovector, 60);

	Context;

	int i;

	//putlog(LOG_MISC, "*", "<DEBUG> regex_filter(): pattern = '%s'; text = '%s'; result = %d; ovector[0] = %d; ovector[1] = %d;", pattern, text, result, ovector[0], ovector[1]);

	if (result > 0) {
		for (i = ovector[0]; i < (strlen(text) - (ovector[1] - ovector[0])); i++)
		   text[i] = text[i + (ovector[1] - ovector[0])];
		text[i] = '\0';
		
		// Fake a global: recursevly call until there are no more matches.
		regex_filter(pattern, text);
	}


	//putlog(LOG_MISC, "*", "<DEBUG> regex_filter(): teststr = '%s'", text);

	pcre_free(regexp);
}

static bool contains_botnick(char *text)
{
	pcre *regexp;
	const char *error;
	int erroffset;
	static char *pattern=nmalloc(sizeof(char)*256);

	Context;

	snprint(pattern, sizeof(pattern), "\\b%s\\b", mbotnick);
	regexp = pcre_compile(pattern, PCRE_CASELESS, &error, &erroffset, NULL);
	if (!regexp) {
	...
	int ovector[6];
	int result = pcre_exec(regexp, NULL, text, strlen(text), 0, 0, ovector, 6);

	pcre_free(regexp);
	nfree(pattern);

	// putlog(LOG_MISC, "*", "<DEBUG> contains_botnick(): text = '%s'; result = %d;", text, result);
...
{
	static char *pattern=nmalloc(sizeof(char)*256);
	//putlog(LOG_MISC, "*", "<DEBUG> clean_input(): input: '%s'", text);

	Context;

	regex_filter("^\\S+: ", text); // Directed statement filter
	regex_filter("\\s:[dp]|\\)|\\(\\W*$", text);  // Simplistic emoticon remover
	regex_filter(" {2,}|!{2,}|\\?{2,}", text); // Adjacent space and punct
	regex_filter("^\\W+", text);
	regex_filter(" +$", text);

	snprint(pattern, sizeof(pattern), "%s[^s]", mbotnick);
	regex_filter(pattern, text); // Botnick scrubber

	nfree(pattern);

	//putlog(LOG_MISC, "*", "<DEBUG> clean_input(): output: '%s'", text);
}
...
static void do_megahal(int idx, char *prefix, char *text, bool learnit, char *nick, char *chan)
{
	...
	//putlog(LOG_MISC, "*", "<DEBUG> do_megahal(%i, %s, %s, %i, %s, %s)", idx, prefix, text, learnit, nick, chan);
...
static int pub_megahal2(char *nick, char *host, char *hand, char *channel, char *text)
{
	...
	for (i=0; i<words->size; i++) // find whether one of the response keywords is in the text - find first only
		if ((keyword = istextinlist2(words->entry[i], responsekeywords)))
			break;
	if (chan != NULL && (/*mystrstr(buffer, mbotnick)*/ contains_botnick(buffer) || keyword) && !flg) { // either botnick or keyword said - respond immediately and exit
		Context;
		// putlog(LOG_MISC, "*", "<DEBUG> do_megahal2 line 771");
		if (!floodcheck()) {
			nfree(buffer);
		  	return 0;
		}
...
static int tcl_megasay STDVAR
{
	Context;
	BADARGS(3, 3, " <channel> <input text>");

	char prefix[strlen(argv[1]) + 13];

	sprintf(prefix, "PRIVMSG %s :", argv[1]);
	do_megahal(DP_HELP, prefix, argv[2], FALSE, NULL, argv[1]);
...
static void capitalize(char *string)
{
	...
	regexp = pcre_compile("^\\W*(\\w)(?:.*?[\\.\\?!]\\s(\\w))*", 0, &error, &erroffset, NULL);
	rc = pcre_exec(regexp, NULL, string, strlen(string), 0, 0, ovector, 60);
	if (rc > 0)	for (i = 1; i < rc; i++) string[ovector[i * 2]] = (char)toupper((int)string[ovector[i * 2]]);
	pcre_free(regexp);

	regexp = pcre_compile("\\bI\\b", PCRE_CASELESS, &error, &erroffset, NULL);
	rc = pcre_exec(regexp, NULL, string, strlen(string), 0, 0, ovector, 60);
	if (rc > 0) for (i = 0; i < rc; i++) string[ovector[i * 2]] = (char)toupper((int)string[ovector[i * 2]]);
	pcre_free(regexp);

	//putlog(LOG_MISC, "*", "<DEBUG> capitalize(): string = '%s'", string);
...
static void make_words(char *pinput, DICTIONARY *words)
{
	...
	while(1) {
		...
		/*	buffer = nmalloc(offset+1);
			for (i=0; i<offset; i++) buffer[i] = input[i];
			buffer[offset] = '\0';
			putlog(LOG_MISC, "*", "make_words: %s", buffer);
			nfree(buffer); */
